...
Agent 单次生成通常只能出 1500-3000 字，远低于 5000 字目标。必须使用分段策略，**不可将一段当成一章提交**。

**流程（每章标准执行）：**

1. **加载锚点**：读本章大纲的场景分段描述 + 玩法铁律（从胶囊读取）+ 角色状态
2. **逐段生成**：
   - 每段只传当前段场景描述 + 前一段的结尾 300 字作为衔接上下文（第一段用章首衔接语）
   - 不传全章总览——每段独立生成，防止指令稀释和风格趋同
3. **每段续写门控**：
   - 生成完后立刻跑 wordcount
   - 低于段目标字数的 70% → 自动续写一轮（传当前段结尾 300 字 + "继续，不要重复已有内容"）
   - 续写最多 3 轮；超过 3 轮仍未达标 → 记录并继续下一段
4. **全章拼接**：所有段落生成完毕 → 按顺序拼接 → 跑 wordcount 验证全章达标
5. **交付门槛（P0 硬约束）**：
   - 全章总字数必须 ≥ 目标字数的 60%（5000 字目标则不低于 3000 字）
   - **不达标不得报告"完成"**，必须回到未达标的段落继续补写
   - 全部达标后才能写入进度.txt 并报告用户

**进度报告规则：**
- 每段生成完成后报告：`第N章-段X/X 完成 (XXXX字，段目标YYYY字)`
- 只有全章拼接并 wordcount 验证达标后，才能将该章标记为 ✅ 并更新进度.txt
- 未达标的章节标记为 🔶，**不得以"已完成"汇报给用户**
...
learned_from_session:
  sample_count: 60
  high_density_works: ["少妇白洁", "白领丽人之地铁", ...]
  technique_profile:
...
- 大纲与正文章节号必须对齐。
- 环境/情节铺垫每章累计不超过 40%。
- **最低字数阀门（P0）**：任何交付给用户的单章字数不得低于目标字数的60%（5000字目标则不低于3000字）。低于阀值的章节必须当场补写至达标，或明确标注为"草稿"征求用户同意跳过。禁止将1300-2000字的场景段落作为一章交付。
